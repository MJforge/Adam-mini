...
│   └── setup.py
├── experiment_1/                         # 효과성(Effectiveness) 재현
...
`experiment_1/train_gpt2.py`는 `original_code/examples/gpt2/`의 `model.py`, `configurator.py`, `logger.py`를 직접 참조한다. 별도 복사본 없이 `sys.path`로 연결되어 있으므로 `original_code`를 삭제하면 안 된다.

각 실험 폴더가 재현하는 논문의 핵심 주장은 다음과 같다.
...
#### experiment_1 — 효과성 (GPT-2 125M 사전학습)
...
옵티마이저는 손실이 줄어드는 방향과 폭, 즉 "어느 방향으로 얼마나 크게 파라미터를 바꿀지"를 정하는 학습 알고리즘이다. Adam(과 그 변형인 AdamW)은 대규모 언어 모델 학습의 사실상 표준이지만, 파라미터마다 1차 모멘텀 `m`과 2차 모멘텀 `v`를 따로 저장한다. 파라미터가 70억 개면 `m`도 `v`도 각각 70억 개를 메모리에 들고 있어야 하고, 분산 학습을 써도 GPU 간 통신 한계 때문에 속도가 떨어진다.

Adam-mini는 여기서 출발한다. 신경망의 Hessian(파라미터 조합에 대한 2차 미분을 모아 놓은 행렬)을 들여다보면 완전히 뒤섞인 형태가 아니라 **블록 대각(block-diagonal)** 구조에 가깝다. 즉 몇 개의 그룹 안에서는 파라미터들이 강하게 상호작용하지만, 그룹 사이의 상호작용은 약하다. Adam-mini는 이 점을 이용해 비슷한 파라미터들을 하나의 블록으로 묶고, 블록마다 학습률(2차 모멘텀 `v`)을 하나만 둔다. 이렇게 하면 `v`의 99.9% 이상을 제거할 수 있고, 옵티마이저 상태 메모리가 약 절반으로 줄어든다.

논문이 내세우는 핵심 주장은 세 가지다.
...
본 프로젝트는 위 세 주장을 재현하는 실험 3개와, 주장을 뒷받침하는 동기 실험(case study) 2개, 그리고 논문의 한계를 직접 건드려 보는 비판적 검증 실험 2개로 구성된다.
...
이 부분은 논문에 코드가 공개되어 있지 않아, 논문과 비슷한 구성의 간단한 신경망을 만들어 확인했다.

- **구성**: MNIST 데이터셋, 은닉층 1개짜리 MLP(은닉 뉴런 8개)를 학습하며 Hessian을 시각화
- **결과**: 그룹 내부의 상호작용은 강하고 그룹 사이의 상호작용은 약한, 블록 대각에 가까운 패턴이 학습 전 구간에서 유지됨
- **의미**: 신경망 파라미터가 모두 강하게 얽혀 있는 것이 아니라, 몇 개의 그룹 단위로 나뉘어 동작한다는 점을 확인. 블록 단위로 학습률을 묶어도 되는 근거가 된다.

### 2-2. Random Quadratic Problem

무작위 2차 함수 최적화 문제에서 단일 학습률 / 블록별 학습률 / 파라미터별 학습률(Adam) 방식을 비교했다.

- **결과**: 전체 문제 차원에서는 파라미터마다 다른 학습률을 주는 편이 도움이 되지만, 하나의 밀집(dense) 블록 안에서는 파라미터별로 학습률을 다르게 줄 필요가 크지 않았다. 잘 고른 학습률 하나로도 충분했다.
- **의미**: "블록 하나당 학습률 하나"라는 Adam-mini의 압축 전략이 성립할 수 있는 조건을 보여준다.
...
**목적.** 모델 크기와 무관하게 Adam-mini의 옵티마이저 상태 메모리가 AdamW 대비 정확히 50% 절감되는지 검증한다.

**조건.** 논문은 80GB VRAM 서버 클러스터에서 측정했으나, 일반 Colab 환경에서 Llama 2-13B나 Llama 3-8B를 올리면 곧바로 OOM이 발생한다. 이를 피하기 위해 PyTorch의 **Meta Device**를 사용했다. Meta Device는 파라미터의 실제 값을 메모리에 올리지 않고 모델의 구조(텐서 형태)만 만들어 두는 가상 장치로, CUDA의 메모리 관리 영향 없이 옵티마이저 상태 메모리만 분리해 계산할 수 있다. 덕분에 서로 다른 크기의 모델을 동일한 조건에서 비교할 수 있다.

**측정 대상.** GPT-2-1.5B, Llama 2-1B, Llama 2-7B, Llama 3-8B, Llama 2-13B
...
**결론.** 프레임워크의 캐싱 노이즈를 배제한 이론적 계산 결과, 모든 스케일에서 논문과 일치하는 50.0% 절감을 재현했다. 실제 GPU에서의 측정은 [Section 4-2](#4-2-physical-gpu-memory-reduction)에서 별도로 다룬다.

### 3-2. Effectiveness

**목적.** GPT-2 125M 사전학습에서 Adam-mini의 validation loss가 AdamW와 초기 구간에서 비슷하게 수렴하는지 확인한다.

**조건.** 논문과 같은 8×A100 80GB 환경은 현실적으로 어려워 단일 GPU로 축소했다. 다만 옵티마이저 간 비교의 공정성을 위해 step당 토큰 수(`tokens/iter`)는 논문과 동일하게 맞췄다.
...
데이터는 17GB에 달하는 전체 OpenWebText를 내려받는 대신 Hugging Face 스트리밍으로 약 1%(80,000개 문서)만 추출했고, 원본과 동일한 포맷(`uint16`)의 `train.bin` / `val.bin`을 생성했다. 따라서 학습 코드(`train_gpt2.py`)는 수정하지 않았다. 두 옵티마이저 모두 완전히 같은 데이터와 같은 배치, 같은 반복 횟수를 거치므로 절대 성능은 낮아질 수 있어도 상대 비교에는 논리적 문제가 없다.
...
**분석 및 결론.** 단일 GPU 제약으로 약 0.7B 토큰까지만 학습했기 때문에, loss 곡선만으로 "완전히 재현했다"고 말하기는 어렵다. 다만 초기 수렴 양상은 원 논문의 그래프와 유사했고, Adam-mini가 더 적은 메모리로 비슷한 수렴과 근소하게 짧은 학습 시간을 보였다. 전체 49B 토큰까지 학습한다면 논문과 같은 곡선에 수렴할 것으로 추정된다.

### 3-3. Efficiency

**목적.** Adam-mini의 메모리 절감이 실제로 "더 큰 batch size 허용 → GPU 간 통신 감소 → 처리량 향상"으로 이어지는지 검증한다.

**조건.** Llama 2-7B를 2×A100-80GB에서 AdamW와 Adam-mini 각각으로 30 step 학습하며, batch size를 1부터 늘려 가며 최대 batch size와 peak throughput을 비교했다.
...
**분석.** 같은 batch=1에서 Adam-mini는 AdamW보다 GPU 메모리를 29.4%p 적게 썼다(49.8% vs 79.2%). 이 여유 덕분에 50% 더 큰 배치(3 vs 2)를 안정적으로 처리했고, 그 결과 3.8% 높은 peak throughput을 달성했다(7,371 vs 7,104 wps). "메모리 절약 → 배치 증가 → 처리량 향상"이라는 논문의 핵심 메커니즘은 그대로 재현됐다.
...
향상률 수치 자체는 논문(+49.6%)과 차이가 크지만, 이는 출발점이 달라진 결과로 본다. 정리하면 다음과 같다.
...
**결론.** 향상률 수치는 환경 차이로 논문에 미치지 못했으나, batch=1 기준 +42.7%로 근접했고, 핵심 메커니즘은 동일하게 재현됐다.
...
Adam-mini의 핵심 가정은 "같은 파티션(블록) 안의 파라미터들은 최적 학습률이 비슷하다"는 것이다. GPT-2 기준으로 Attention Q/K/V/O는 head 단위, MLP는 뉴런(output row) 단위, Embedding은 전체를 하나의 파티션으로 묶는다. 즉 파티셔닝 자체가 Transformer 구조를 알고 있다는 가정 위에 서 있다.

이 가정이 얼마나 민감한지 보기 위해, 효과성 실험과 동일한 조건에서 `n_head`만 12에서 6으로 줄여(블록을 더 크고 거칠게 나눠) GPT-2 125M을 학습했다.
...
블록을 절반으로 거칠게 나누자 AdamW와의 격차가 0.155에서 0.243으로 약 57% 벌어졌다. 파티션을 거칠게 나눌수록 Adam-mini가 불리해진다는 뜻으로, 성능이 파티셔닝 방식과 정밀도에 민감하다는 논문의 한계를 실험적으로 뒷받침한다. MoE처럼 구조가 다르거나 파티션 단위가 큰 모델에 적용할 때는 이 민감도를 고려해야 한다.

### 4-2. Physical GPU Memory Reduction

경량성 실험은 Meta Device로 측정한 이론값이다. 이 50% 절감이 실제 GPU에서도 동일하게 나타나는지 확인했다.

- **방법**: Google Colab의 T4 GPU에서 GPT-2 모델의 `optimizer.step()` 전후 VRAM 변화를 측정. 현업에서 많이 쓰는 half precision까지 보기 위해 float32와 float16 두 조건에서 측정
- **결과**:
...
연산 정밀도와 무관하게 실제 학습 파이프라인에서도 약 50%의 절감이 유지됐다. 이론 측정값과 실측값이 일치함을 확인했다.
...
- **효율성**: Adam-mini의 메모리 절약(29.4%p)이 더 큰 배치(2→3)로 이어져 3.8% 높은 peak throughput을 달성했다. batch=1 기준으로는 +42.7%로 논문에 근접했고, 논문의 핵심 메커니즘이 동일하게 작동함을 확인했다.
...
- 효율성 실험은 논문 시점 이후 PyTorch의 GPU 메모리 관리 효율이 개선되어, 논문과 완전히 동일한 환경·결과를 재현하지는 못했다.
